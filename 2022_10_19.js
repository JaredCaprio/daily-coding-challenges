...
const cannonsReady = (gunners) => {
  let gunnersAns = Object.values(gunners);
  let results = [];
  for (let i = 0; i < gunnersAns.length; i++) {
    if (gunnersAns[i] === "aye") {
      results.push(gunnersAns[i]);
    }
  }
  return results.length == gunnersAns.length ? "Fire!" : "Shiver me timbers!";
};
